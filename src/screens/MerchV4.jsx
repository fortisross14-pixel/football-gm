...
export default function MerchV4({game,produce,upgradeChannel,setPrice}){
  ...
  return <><SectionTitle eyebrow="Negocio · Merchandising" title="Producto, stock y canales" action={<Pill>{inv.toLocaleString('es-ES')}/{cap.toLocaleString('es-ES')} uds almacén</Pill>}/>
    <div className="merch-v3-hero"><img src={merchArt} alt=""/><div><span>OPERACIÓN DE RETAIL</span><h2>Fabricar requiere dinero, tiempo y almacén</h2><p>Los pedidos tardan varios días en llegar. El capital queda comprometido desde que firmas la orden y después inmovilizado en inventario. La demanda depende de aficionados, resultados, diseño de la equipación y capacidad de los canales.</p></div><Metric label="Ventas temporada" value={money(game.merch.revenueSeason)} sub={game.kit?.manufacturer?`${share}% para el club según fabricante`:'Sin fabricante contratado'}/></div>
    {(game.processes||[]).some(p=>p.type==='merchProduction')&&<div className="production-strip">{game.processes.filter(p=>p.type==='merchProduction').map(p=><div key={p.id}><b>Pedido en fábrica · {p.payload.qty} uds</b><span>{Math.max(0,p.dueDay-game.day)} días para entrega</span></div>)}</div>}<h3 className="minor-title">Catálogo y producción</h3><div className="product-v3-grid">{products.map(([id,name,cost])=><article key={id}><div className={`product-visual pv-${id}`}><span>{name}</span></div><h3>{name}</h3><div className="product-stats"><span>Stock <b>{game.merch.inventory[id]}</b></span><span>Coste base <b>{money(cost)}</b></span></div><label>Precio de venta<input type="number" min="1" value={game.merch.prices[id]} onChange={e=>setPrice(id,e.target.value)}/></label><label>Lote a fabricar<input type="number" min="1" step="50" value={qty[id]} onChange={e=>setQty({...qty,[id]:Number(e.target.value)})}/></label><button className="primary wide" onClick={()=>produce(id,qty[id])}>Encargar {qty[id]} uds</button></article>)}</div>
