...
import {
  createGame,advanceDays,isMatchReady,simulateCurrentMatch,activateSponsorRight,produceMerch,startSponsorSearch,submitSponsorNegotiation,signSponsor,
...
function advanceForQA(input,days){let g=input,target=g.day+days,guard=0;while(g.day<target&&guard++<500){const beforeDay=g.day;g=advanceDays(g,target-g.day);if(isMatchReady(g)){g=simulateCurrentMatch(g).game;continue;}if(g.day===beforeDay)break;}return g;}
...
assert(isMatchReady(game),'Calendar should stop on match day before simulating');
assert(game.history.length===0,'Match must not auto-simulate when time reaches match day');
const sim=simulateCurrentMatch(game);assert(sim.timeline.length===6,'Match simulation should expose six 15-minute ticks');game=sim.game;
assert(game.history.length===1,'Match simulation did not settle first match');
assert(game.players.some(p=>p.stats.apps>0),'Player match statistics were not allocated');
assert(game.matchReport?.lineup?.length===11&&game.matchReport.teamStars>=1&&game.matchReport.teamStars<=5,'Post-match report invalid');
const tp=ticketProjection(game);assert(tp.attendance>=0&&tp.attendance<=game.stadium.capacity,'Ticket projection invalid');
const stockBefore=game.merch.inventory.shirt;game=produceMerch(game,'shirt',100);assert(game.merch.inventory.shirt===stockBefore&&game.processes.some(p=>p.type==='merchProduction'),'Merch production should be asynchronous');game=advanceForQA(game,5);assert(game.merch.inventory.shirt>=stockBefore+100,'Merch production did not arrive after lead time');

// Sponsors are a timed search, followed by next-day negotiation.
game=activateSponsorRight(game,'shirtMain');game=startSponsorSearch(game,'shirtMain','local',60);
assert(game.processes.some(p=>p.type==='sponsorSearch'),'Sponsor search did not start');
game=advanceForQA(game,60);
...
game=advanceForQA(game,14);
const phys=game.staffCandidatePools['Preparador físico']?.[0];assert(phys,'Staff search returned no candidates');
game=submitStaffNegotiation(game,'Preparador físico',phys.id,phys.salary,[]);game=advanceForQA(game,1);
...
game=startManufacturerSearch(game,14);game=advanceForQA(game,14);
...
if(target>current){
  ...
  game=advanceForQA(game,totalWeeks*7);
...
game=submitPlayerOffer(game,player.id,player.agent.salaryAsk,player.agent.signingAsk);game=advanceForQA(game,1);
...
const conditionBefore=game.stadium.condition;game=scheduleEvent({...game,cash:game.cash+1_000_000},'concert',date);game=advanceForQA(game,7);
...
game=startInvestment({...game,cash:game.cash+1_000_000},'low',100_000,365);game=advanceForQA(game,365);
...
let saved=await saveGameToSlot(2,'Mi Lugo móvil',game);assert(saved.ok,'Save slot write failed');
let loaded=await loadGameFromSlot(2);assert(loaded.ok&&loaded.game.club.id===game.club.id,'Save slot load failed');
let summaries=await getSlotSummaries();assert(summaries.filter(s=>!s.empty).length===1,'Save index should contain exactly one test slot');
let renamed=await renameSaveSlot(2,'Ascenso con Lugo');summaries=await getSlotSummaries();assert(renamed.ok&&summaries.find(s=>s.id===2).slotName==='Ascenso con Lugo','Save slot rename failed');
let deleted=await deleteSaveSlot(2);summaries=await getSlotSummaries();assert(deleted.ok&&summaries.find(s=>s.id===2).empty,'Save slot delete failed');

console.log('v0.4.3 smoke tests passed',{
